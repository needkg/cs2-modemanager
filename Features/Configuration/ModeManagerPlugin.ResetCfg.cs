...
{
    private void EnsureResetCfgFileExists()
    {
        try
        {
            if (!TryExtractExecCfgRelativePath(Config.ResetCommand, out var relativeCfgPath))
                return;
            ...
            LogInfo(Msg(MessageKey.LogResetCfgCreated, path));
        }
        catch (Exception ex)
        {
            LogError(Msg(MessageKey.LogResetCfgCreateFailed, Config.ResetCommand, ex.Message));
        }
    }

    private string? ResolveResetCfgPhysicalPath(string relativeCfgPath)
    {
        var discoveredConfigPath = ConfigPathDiscovery.Discover(ModuleName, nameof(ModeManagerPlugin));
